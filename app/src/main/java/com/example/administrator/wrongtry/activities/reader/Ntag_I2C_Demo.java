...
import com.example.administrator.wrongtry.activities.activities.FlashMemoryActivity;
...
import com.example.administrator.wrongtry.activities.reader.Ntag_I2C_Commands.Register;

import static com.example.administrator.wrongtry.activities.reader.I2C_Enabled_Commands.*;
...
public class Ntag_I2C_Demo implements WriteEEPROMListener, WriteSRAMListener {
    ...
    public Ntag_I2C_Demo(Tag tag, final Activity main, final byte[] passwd, final int authStatus) {
        try {
            ...
            if (!prod.equals(Prod.Unknown)) {
                if (prod.equals(Prod.NTAG_I2C_1k_Plus)
                        || prod.equals(Prod.NTAG_I2C_2k_Plus)) {
...
//	}
    ...
    public void NDEFReadFinish() {
        if (ndefreadtask != null && !ndefreadtask.isCancelled()) {
            ...
            ndefreadtask = null;
            //Toast.makeText(main, "reset一下", Toast.LENGTH_LONG).show();
...
//				}
                Toast.makeText(main, "写入成功", Toast.LENGTH_LONG).show();
            ...
            // 看这里的代码
            // 选择的是读操作
            ndefreadtask = new NDEFReadTask();// new一个异步任务对象
            ndefreadtask.execute();// 执行异步任务,触发执行
        }
    }

    // 读操作
    // AsyncTask是一个抽象类,此处使用子类NDEFReadTask继承
    // 3个泛型参数:启动任务执行的输入参数 后台任务执行的进度 后台计算结果的类型
    // 此处使用String数据作为进度显示单位
    private class NDEFReadTask extends AsyncTask<Void, String, Void> {

        private Boolean exit = false;

        @Override
        protected Void doInBackground(Void... params) {// 在onPreExecute()后立即执行,用于执行较为费时的操作
            Log.i("Ntag_I2C_Demo","start doInBackground");// 此处只执行一次
            try {
                while (true) {
                    Log.i("Ntag_I2C_Demo","doInBackground Loop");
                    ...
                    long timeToReadNdef = System.currentTimeMillis() - RegTimeOutStart;// 计算时差
                    ...
                    if (!highLevelMsg.isEmpty()) {
                        type = highLevelMsg.get(0).getClass().getSimpleName();// 获取Message的类型
                    }

                    String message = "";
                    int num_record = 1;
                    // 以下for循环用于读取Message的内容,存放于message变量中
                    ...
                    // 在执行doInBackground中可以调用publishProgress来更新进度信息
                    // Put the message content on the screen
                    publishProgress(type, message, readTimeMessage);// 3个参数传到onProgressUpdate中
                ...
                Log.i("Ntag_I2C_Demo","CommandNotSupportedException");
                e.printStackTrace();
                showAlert(
                ...
                Log.i("Ntag_I2C_Demo","Exception");
                e.printStackTrace();
            }
            return null;
        }

        // onProgressUpdate在调用publishProgress时被执行,直接将进度信息显示在UI组件上
        @Override
        protected void onProgressUpdate(String... progress) {
            Log.i("Ntag_I2C_Demo","onProgressUpdate函数正在执行");
        ...
        @Override
        protected void onPostExecute(Void bla) {// 在后台操作结束时被调用
            Log.i("Ntag_I2C_Demo","onPostExecute");
			/*if (!exit) {
				Toast.makeText(main, main.getString(R.string.Tag_lost),
						Toast.LENGTH_LONG).show();
			}*/
            NdefFragment.setndefwrongflag();// 相当于再点击一次按钮
            NdefFragment.resetNdefDemo();// 此函数暂时没有操作
        }
    }
