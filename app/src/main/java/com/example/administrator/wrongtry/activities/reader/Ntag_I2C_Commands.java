...
****************************************************************************
*/
package com.example.administrator.wrongtry.activities.reader;
...
import com.example.administrator.wrongtry.activities.activities.AuthActivity;
...
import com.example.administrator.wrongtry.activities.exceptions.CC_differException;
import com.example.administrator.wrongtry.activities.exceptions.CommandNotSupportedException;
import com.example.administrator.wrongtry.activities.exceptions.DynamicLockBitsException;
import com.example.administrator.wrongtry.activities.exceptions.NotPlusTagException;
import com.example.administrator.wrongtry.activities.exceptions.StaticLockBitsException;
import com.example.administrator.wrongtry.activities.listeners.WriteEEPROMListener;
import com.example.administrator.wrongtry.activities.listeners.WriteSRAMListener;
import com.example.administrator.wrongtry.activities.reader.Ntag_Get_Version.Prod;
...
public class Ntag_I2C_Commands extends I2C_Enabled_Commands {
	...
	public Ntag_I2C_Commands(Tag tag) throws IOException {
		...
		blockSize = SRAM_BLOCK_SIZE;//4
		SRAMSize = SRAM_SIZE;//64???
		this.reader = new Ntag_Commands(tag);//newһ������
		this.tag = tag;
		connect();
		//���캯�����ñ��ط���getProduct()
		if (Prod.NTAG_I2C_2k == getProduct()) {
			sramSector = 1;//2k�����
		} else {
			sramSector = 0;//1k�����
		}
	...
	public Prod getProduct() throws IOException {
		//getVersionResponse�ǵ�ǰ���һ����Ա����
		if (null == getVersionResponse) {//�ж��Ƿ�Ϊ��
			try {
				//Ntag_Get_Version��Ĺ��캯������Ĳ�������P48�е���8���ֽ�
				//����������׳��쳣�������������,���쳣��Ϣ�洢����
				//���û�з����쳣��������catch�Ӿ��е����
				getVersionResponse = new Ntag_Get_Version(reader.getVersion());//newһ������
			} catch (Exception e) {
				e.printStackTrace();
				try {
					reader.close();
					reader.connect();//��������
					byte[] temp = reader.read((byte) 0x00);//��ȡǰ4��Page
					...
					if (temp[0] == (byte) 0x04 && temp[12] == (byte) 0xE1
						...
						temp = reader.read((byte) 0xE8);//��ȡ���üĴ���
						//������if����е�������˵������1k����
	...
	public byte[] getSessionRegisters() throws IOException, FormatException {
		if (getProduct() == Prod.NTAG_I2C_1k_Plus || getProduct() == Prod.NTAG_I2C_2k_Plus) {
			...
			//getValue?????????????��?????????????????
			return reader.read(Register.Session_PLUS.getValue());//reader????????????new????????
		} else {
...
//		}
	...
	public void writeEEPROM(byte[] data, WriteEEPROMListener listener) throws IOException, FormatException {
		...
		reader.SectorSelect((byte) 0);//ѡ���һ��sector����д��
		byte[] temp;
		int index = 0;
		byte blockNr = Register.User_memory_Begin.getValue();//0x04
		...
		// �����1��sectorд���¾�д����2��sector��
		// If Data is left write to the 1. Sector
		if (index < data.length) {// �����û��д��
			reader.SectorSelect((byte) 1);// ѡ���2��sector
			blockNr = 0;
	...
	public void writeEEPROM(int startAddr, byte[] data) throws IOException,
		...
		reader.SectorSelect((byte) ((startAddr & 0x200) >> 16));//0
		byte[] temp;
		int index = 0;
		byte blockNr = (byte) (startAddr & 0xFF);//blockNr = 0x XX

		// write till all Data is written or the Block 0xFF was written(BlockNr
		// should be
		// 0 then, because of the type byte)
		for (index = 0; index < data.length && blockNr != 0; index += 4) {
			//temp�Ĵ�СΪ4���ֽ�,һ��д��4���ֽ�
			temp = Arrays.copyOfRange(data, index, index + 4);
			reader.write(temp, blockNr);//һ��д��1��temp��4���ֽ�
			blockNr++;
		}

		// If Data is left write and the first Sector was not already written
		// switch to the first
		// �����0��sector�Ѿ�ȫ��д��,��ʼд���1��sector
		if (index < data.length && (startAddr & 0x100) != 0x100) {
			reader.SectorSelect((byte) 1);//ѡ���1��sector
			blockNr = 0;//blockNr��0
			for (; index < data.length; index += 4) {
				//temp�Ĵ�СΪ4���ֽ�,һ��д��4���ֽ�
				temp = Arrays.copyOfRange(data, index, index + 4);
		...
		} else if ((startAddr & 0x100) == 0x100) {//�����8λΪ1
			throw new IOException("Data is to long");
		}
	}
	...
	public byte[] readEEPROM(int absStart, int absEnd) throws IOException,
		...
		while (fetchStart <= absEnd) {
			//ÿһ��whileѭ����Ҫȥfetchһ�Σ���fetchStart��fetchEnd
			fetchEnd = fetchStart + maxFastRead - 1;//���ε�fetchEnd
			...
			} else {//��getProduct() = Prod.NTAG_I2C_2k_Plus��ִ�����´���
			...
			fetchStart = fetchEnd + 1;//����fetchStart
	...
	public void writeSRAMBlock(byte[] data, WriteSRAMListener listener) throws IOException, FormatException {
		...
		int index = 0;
		reader.SectorSelect(sramSector);//0
		...
		if((getProduct() == Prod.NTAG_I2C_1k_Plus || getProduct() == Prod.NTAG_I2C_2k_Plus)
			...
			reader.fast_write(data, (byte) Register.SRAM_Begin.getValue(),
					(byte) (Register.SRAM_Begin.getValue() + 0x0F));//��??16?????
		} else {
			if (getProduct() == Prod.NTAG_I2C_1k) {
				...
				if (ul != null) {
					...
					//SRAM��64���ֽ�,��Ϊ16��Page,ÿ��Page��4���ֽ�
					for (int i = 0; i < 16; i++) {//ÿһ��Pageִ��һ��
						for (int dI = 0; dI < 4; dI++) {//һ��Page�����ÿһ���ֽ�ִ��һ��
							if (index < data.length) {
								txBuffer[dI] = data[index++];//txBuffer�Ĵ�СΪ4���ֽ�
							} else {
								txBuffer[dI] = (byte) 0x00;
							}
						}
						ul.writePage(SRAM_Begin + i, txBuffer);//д��һ��Page
					}
					ul.close();
				}
				reader.connect();
	...
	public byte[] readSRAM(int blocks, R_W_Methods method) throws IOException,
		...
		for (int i = 0; i < blocks; i++) {
			...
			response = concat(response, temp);//???����Ӧ����ôд
		}
		answer = response;
		return response;
	}
	...
	public void writeDefaultNdef() throws IOException, FormatException {
		...
		reader.SectorSelect((byte) 0);//ѡ��Sector
	...
	public NdefMessage readNDEF() throws IOException, FormatException {
		...
		byte[] tlv = readEEPROM(Register.User_memory_Begin.getValue(),
				Register.User_memory_Begin.getValue() + 3);//��ȡǰ4���ֽ�Ϊtlv

		// checking TLV - maybe there are other TLVs on the tag
		if (tlv[0] != 0x03) {//�����1���ֽڲ��ǿ�ʼ�ֽ�
			throw new FormatException("Format on Tag not supported");
		}
		if (tlv[1] != (byte) 0xFF) {//�����2���ֽڲ���0xFF,˵��ֻ��1���ֽڱ�ʾsize
			ndefsize = (tlv[1] & 0xFF);//ndef��sizeΪtlv[1]
			tlvsize = 2;//tlv��sizeΪ2,�ֱ�����ʼλ��sizeλ��1λ
			tlvPlusNdef = tlvsize + ndefsize;//�ܵĴ�С
		} else {
			ndefsize = (tlv[3] & 0xFF);//��8λ
			ndefsize |= ((tlv[2] << 8) & 0xFF00);//��8λ
			tlvsize = 4;//tlv��sizeΪ4
			tlvPlusNdef = tlvsize + ndefsize;//�ܵĴ�С
		}
		// Read NDEF Message
	...
	private byte[] createRawNdefTlv(NdefMessage NDEFmessage)
		...
		if (ndefMessageSize < 0xFF) {
			...
			TLV_size |= 0xFF0000;//int��4���ֽ�,��3���ֽ�ȫ��д1��Ϊ���
			message[0] = (byte) 0x03;//��ʼ
			message[1] = (byte) ((TLV_size >> 16) & 0xFF);//��3���ֽ�Ϊȫ1
			message[2] = (byte) ((TLV_size >> 8) & 0xFF);//�洢size�ĵ�2���ֽ�
			message[3] = (byte) (TLV_size & 0xFF);//�洢size�ĵ�1���ֽ�
			message[message.length - 1] = (byte) 0xFE;//����
