...
public class Ntag_Commands {
	...
	public Ntag_Commands(Tag tag) throws IOException {
		nfca = NfcA.get(tag);
		sectorSelectTimout = timeout;//��ʼ��Ϊ20
		nfca.setTimeout(timeout);//����Timeout
		currentSec = 0;
	}
	...
	public boolean isConnected() {
		return nfca.isConnected();//�ж��Ƿ񻹿��Խ���IO����
	}
	...
	public void SectorSelect(byte sector) throws IOException, FormatException {
		...
		command = new byte[2];
		command[0] = (byte) 0xc2;//SECTOR SELECT
		command[1] = (byte) 0xff;//0xFF
	...
	public void fast_write(byte[] data, byte startAddr, byte endAddr) throws IOException,
		...
		command[0] = (byte) 0xA6;//д������SRAM
		command[1] = (byte) startAddr;//��ʼ��ַ
		command[2] = (byte) endAddr;//������ַ
	...
	public void write(byte[] data, byte blockNr) throws IOException,
			FormatException {
		// no answer
		answer = new byte[0];//no answer���Բ�����0
		command = new byte[6];
		command[0] = (byte) 0xA2;//writeָ��
		command[1] = blockNr;//Page��ַ
		//��4�ֽڵ�����
		command[2] = data[0];
		command[3] = data[1];
		command[4] = data[2];
	...
	public byte[] read(byte blockNr) throws IOException, FormatException {
		command = new byte[2];
		command[0] = (byte) 0x30;//READ
		command[1] = blockNr;//��Ҫ��ȡ����ʼ��ַ
		answer = nfca.transceive(command);
		return answer;
	}
	...
	public byte[] getVersion() throws IOException {
		command = new byte[1];
		command[0] = (byte) 0x60;//GET_VERSIONָ��
		answer = nfca.transceive(command);
		return answer;
	}
