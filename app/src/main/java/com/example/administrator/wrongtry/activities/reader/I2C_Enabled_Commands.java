...
public abstract class I2C_Enabled_Commands {
	 ...
	 */
    //static修饰的方法不依赖于特定的实例,被类的所有实例共享
	...
	public static I2C_Enabled_Commands get(Tag tag) throws IOException,
		...
		nfca.setTimeout(20);//设置transceive(byte[])函数的以毫秒为单位的延时
		// check for timeout
		if (nfca.getTimeout() < 50) {//获取当前transceive(byte[])函数的以毫秒为单位的延时
			// check if GetVersion is supported
			try {
                //不能在主应用程序线程调用connect函数
				nfca.connect();
                //1个byte的指令
				command = new byte[1];
				// GET_VERSION
                ...
                //创建Ntag_Get_Version类的无名对象
                //使用answer作为构造函数的参数,调用Get_Product方法
				...
				if (prod == Prod.NTAG_I2C_1k || prod == Prod.NTAG_I2C_2k
                    ...
                    //如果是以上NXP产品中的一种,则说明没问题,返回一个以tag作为构造函数参数的Ntag_I2C_Commands对象
				}
			} catch (Exception e) {
				e.printStackTrace();
				nfca.close();
				// check if sector select is supported
                //如果发现有异常，试试sector select指令能否被支持
				try {
					nfca.connect();
					command = new byte[2];
        ...
        //使用get(tag)获取一个MifareUltralight对象
	...
	public abstract byte getConfigRegister(CR_Offset off) throws IOException,
			FormatException, CommandNotSupportedException, CommandNotSupportedException;
